...
def generate_supplier_monthly_report(cursor, month):
    if len(month) != 7 or not month.startswith('20'):
        print("Error: Please use valid format YYYY-MM (e.g., 2025-11)")
        return False
...
def view_supplier_products_detail(cursor):
    supplier_id = input("Enter Supplier ID (1-20) or press Enter for all: ").strip()
    if supplier_id:
...
def view_employee_details(cursor):
    emp_id = input("Enter Employee ID (1-50) or press Enter for all: ").strip()
    if emp_id:
...
def view_warehouse_employee_distribution(cursor):
    wh_id = input("Enter Warehouse ID (1-5) or press Enter for all: ").strip()
    if wh_id:
...
def check_employee_in_warehouse(cursor):
    emp_id = input("Enter Employee ID (1-50): ").strip()
    wh_id = input("Enter Warehouse ID (1-5): ").strip()

    if not emp_id or not wh_id:
        print("Invalid input")
        return None
    try:
        emp_id_int = int(emp_id)
        wh_id_int = int(wh_id)
        if emp_id_int <= 0 or wh_id_int <= 0:
            print("Invalid input")
            return None
...
while True:
    ...
    if opt=="8":
        ...
        wid=input("warehouse_id (Enter=all, range 1~5):").strip()
        ...
        pid=input("product_id (range 1~100):").strip()
        ...
        month=input("month(YYYY-MM), (range 2025-11 - 2025-12)):").strip()
        ...
        wid=input("warehouse_id(-1=all, range 1~5):").strip()
        pid=input("product_id(-1=all, range 1~100):").strip()
        ...
        try:
            days=input("days_window(e.g. 10):").strip()
